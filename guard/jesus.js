...
const jesus1 = require("discord.js");
const jesus2 = require("discord.js");
const jesus3 = require("discord.js");

const client1 = new jesus1.Client();
const client2 = new jesus2.Client();
const client3 = new jesus3.Client();

const ayarlar = require('./ayarlar.json');
const config = require('./jesus.json');

/////////////////////////////////////////////////////////////////////////////////////////////////////////////
client1.on("ready", async () => {
  let sesliKanalID = client1.channels.cache.get("ses kanal id gir ananı sikmim");
  if (sesliKanalID)
    sesliKanalID.join()
      .catch(err => console.error("Bot ses kanalına bağlanamadı!"));
});
client2.on("ready", async () => {
  let sesliKanalID = client2.channels.cache.get("ses kanal id gir ananı sikmim");
  if (sesliKanalID)
    sesliKanalID.join()
      .catch(err => console.error("Bot ses kanalına bağlanamadı!"));
});
client3.on("ready", async () => {
  let sesliKanalID = client3.channels.cache.get("ses kanal id gir ananı sikmim");
  if (sesliKanalID)
    sesliKanalID.join()
      .catch(err => console.error("Bot ses kanalına bağlanamadı!"));
});
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.roles.cache.forEach(async function(jesus) {
  if (jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});     

  guild.members.ban(entry.executor.id, {reason: "Jesus System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	

  let channel = client1.channels.cache.get(ayarlar.defenderlog)
   if (!channel) return console.log('Ban Koruma Logu Yok!');

   const jesus = new Discord.MessageEmbed()
   .setTimestamp()
  ...
  channel.send(`@everyone`, {embed: jesus}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.roles.cache.forEach(async function(jesus) {
  if (jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});     

  kickhammer.guild.members.ban(yetkili.id, {reason: "Jesus System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  if (member.user.bot) {
  member.guild.roles.cache.forEach(async function(jesus) {
  if (jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});

  member.guild.members.ban(entry.executor.id, {reason: "Jesus System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "Jesus System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.roles.cache.forEach(async function(jesus) {
  if (jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});

  newGuild.members.ban(entry.executor.id, {reason: "Jesus System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Jesus System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.roles.cache.forEach(async function(jesus) {
  if(jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});  

  role.guild.members.ban(entry.executor.id, { reason: `Jesus System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Jesus System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: "Jesus System | Rol Koruma Sistemi" }).catch(e => { })	
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.roles.cache.forEach(async function(jesus) {
  if(jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});  

  newRole.guild.members.ban(entry.executor.id, { reason: `Jesus System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.roles.cache.forEach(async function(jesus) {
  if(jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});

  channel.guild.members.ban(entry.executor.id, { reason: `Jesus System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Jesus System | Kanal Korum Sistemi!" }).then(async kanal => {
...
channel2.send(`@everyone`, {embed: jesus}).catch(e => { })
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Jesus System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Jesus System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.cache.roles.cache.forEach(async function(jesus) {
  if(jesus.permissions.has("ADMINISTRATOR") || jesus.permissions.has("BAN_MEMBERS") || jesus.permissions.has("MANAGE_GUILD") || jesus.permissions.has("KICK_MEMBERS") || jesus.permissions.has("MANAGE_ROLES") || jesus.permissions.has("MANAGE_CHANNELS")) {
  jesus.setPermissions(0).catch(err =>{});}});

  newChannel.guild.members.ban(member.id, { reason: `Jesus System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Jesus System | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: "Jesus System | İzinsiz Webhook Açmak!"}).catch(err => { });
...
client3.on("emojiDelete", async (emoji, message) => {
  ...
  channel.send(`@everyone`, {embed: jesus}).catch(err => { })
return;
});
